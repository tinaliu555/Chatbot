...
// DialogFlow API KEY
// const APIAI_TOKEN='eb22d927e6f847eead5d2954fff072b5'//Alice (original): TaiwanMobile
const APIAI_TOKEN='a8c697c88fb74dcb91ecbb0701c16e4f'//Felix repository: Final_Demo(TaiwanMobile)
...
//User Record
// let userFile = 'Database/Emily.json'
let userFile = 'Database/Nancy.json'
...
io.on('connection', function(socket) {
  socket.on('chat message', (text) => {
    ...
    apiaiReq.on('response', (response) => {
      ...
      if(aiIntent === "momo_shopping" || aiIntent === "cheaper product"){
        ...
        try {
          ...
          if(aiIntent === "momo_shopping"){
            ...
            console.log("aiParameter_product:"+aiParameter_product);
          }else{
            // search for the same product but with different adj, but this intent (cheap first) is remove in Dialogflow now!
            aiParameter_product = userProfile.last_buy;
            //再次確認"cheaper product"中有沒有想搜尋的產品
            if( hasSubString(text,"的")||hasSubString(text,"買")||hasSubString(text,"購")||hasSubString(text,"訂")||hasSubString(text,"找")||hasSubString(text,"查")||hasSubString(text,"搜")||hasSubString(text,"尋")){
              let temp = text.substring(find_index+1);
              if(!isNullOrEmpty(temp)){
                aiParameter_product = temp;
              }
            }
          }
          if(!isNullOrEmpty(aiParameter_product)){
            // the product from the entity
          ...
          }else if( hasSubString(text,"的")||hasSubString(text,"買")||hasSubString(text,"購")||hasSubString(text,"訂")||hasSubString(text,"找")||hasSubString(text,"查")||hasSubString(text,"尋")||hasSubString(text,"搜")){
            // the product in the sentence (text)
          ...
          }else if(!isNullOrEmpty(userProfile.last_buy)){
            // the product last buy
            console.log("user last-buy:" + userProfile.last_buy);
            aiParameter_product = userProfile.last_buy;
            getProductFromMomo(aiParameter_product,getSearchOrderToMomo(adj_product),original_adj,"Buy");
            userProfile.last_buy = aiParameter_product;            
          }else {            
          ...
          //below are used before, it can be check before remove it!
          // if( hasSubString(text,"的")||hasSubString(text,"買")||hasSubString(text,"購")||hasSubString(text,"訂")||hasSubString(text,"找")||hasSubString(text,"查")||hasSubString(text,"尋")||hasSubString(text,"搜")){
      ...
      }else if(aiIntent === "weather answer"){ 
        try {
          let browse_product = userProfile.browse;
          let sale_product = userProfile.sale;
          let responseString = "好的，另外還有一件事，由於您一周前在 MOMO 上瀏覽過" + browse_product +
          "，在這邊向您推薦正在特價中的" + sale_product + "。請問您有興趣瞭解嗎？"
      ...
      }else if(aiIntent === "Difference between two products"){ 
        let adj_product = "合適";
        try {
          let browse_product = userProfile.browse;
          let sale_product = userProfile.sale;
          console.log("browse_product:"+browse_product+", sale_product:"+sale_product);
          if(!isNullOrEmpty(browse_product) && !isNullOrEmpty(sale_product)){
            // console.log("aiIntent === Price||aiIntent === price_test"+aiIntent);
            getProductFromMomoCompare(browse_product,sale_product,getSearchOrderToMomo(adj_product),adj_product,"Disc");
          }else {
            throw new UserException('InvalidProduct');
          }
      ...
      }else if(aiIntent === "Price"){//TODO- need to prevent searching same product
        let adj_product = "合適";
        try {
          let browse_product = userProfile.browse;
          let sale_product = userProfile.sale;
          ...
          if(!isNullOrEmpty(browse_product) && !isNullOrEmpty(sale_product)){
            // console.log("aiIntent === Price||aiIntent === price_test"+aiIntent);
            getProductFromMomoCompare(browse_product,sale_product,getSearchOrderToMomo(adj_product),adj_product,"Price");
          }else {
            throw new UserException('InvalidProduct');
          }
      ...
      }else if(aiIntent === "finish"||aiIntent === "Buy lancome - yes"){
        try{
          socket.emit('search reply', "好的，收件地址為" +userProfile.address + "，已從您 MOMO 會員綁定的信用卡申請扣款。請問還有什麼可以為您服務的嗎？");
        }catch(e){
          console.log(e.message, e.name); // pass exception object to err handler
          socket.emit('search reply', aiText);
        }       
        ...
        try{
          ...
          socket.emit('search reply', "已幫您向主管請假!");
        }catch(e){
          console.log(e.message, e.name); // pass exception object to err handler
          socket.emit('search reply', aiText);
        }
  ...
  function getProductFromMomo(product,adjProduct,org_adjProduct,task){
    socket.emit('search reply', "正在幫您搜尋"+product+"中，請稍後...");
  ...
  let global_sale="";
  let global_result;
  // 到Momo手機板官網爬商品資訊，回傳兩個物品的搜尋結果
  // overall:總和排序, cheapFirst:價格有低到高, expensiveFirst:價格有高到低, sellFirst:銷量排行
  // python momo.py 衛生紙 overall/cheapFirst/expensiveFirst/sellFirst 精華液
  function getProductFromMomoCompare(browse_product,sale_product,adjProduct,org_adjProduct,action){ 
    socket.emit('search reply', "正在幫您搜尋"+browse_product+"與"+ sale_product + "中，請稍後...");
    console.log("[getProductFromMomoCompare] global_browse="+global_browse+ ", global_sale="+global_sale);
    if(global_browse===browse_product && global_sale===sale_product){//不重新搜尋
      let responseString;
        if(action === "Disc"){
          responseString = browse_product + global_result[0].disc + sale_product + global_result[1].disc +"兩款都是 MOMO 人氣排行前三名的商品。";
        }else{
          responseString = "現在"+browse_product+"從原價" + global_result[0].price +"元特價變為"+global_result[0].specialPrice+"元，而"+sale_product+"原價"+global_result[1].price+"元，現在也有驚爆價，" + global_result[1].specialPrice +"元。此外，您有未使用的 MOMO 三百元折價券可以使用於此次交易。";
        }  
        console.log("responseString:"+responseString);
        socket.emit('search reply', responseString);
        socket.emit('img reply', global_result[0].img_url);
        return;
    }
    
    global_browse = browse_product;
    global_sale = sale_product;
    var spawn = require('child_process').spawn,
    py    = spawn('python', ['momo.py',browse_product,adjProduct,sale_product]);
    console.log("[getProductFromMomoCompare] command: python momo.py "+browse_product+" "+adjProduct+" "+sale_product);
    ...
    py.stdout.on('end', function(){      
      try {
        ...
        console.log('[getProductFromMomoCompare] result is '+JSON.stringify(result));
        let responseString;
        if(action === "Disc"){
          responseString = browse_product + result[0].disc + sale_product + result[1].disc +"兩款都是 MOMO 人氣排行前三名的商品。";
        }else{
          responseString = "現在"+browse_product+"從原價" + result[0].price +"元特價變為"+result[0].specialPrice+"元，而"+sale_product+"原價"+result[1].price+"元，現在也有驚爆價，" + result[1].specialPrice +"元。此外，您有未使用的 MOMO 三百元折價券可以使用於此次交易。";
        }  
        console.log("responseString:"+responseString);
        socket.emit('search reply', responseString);
        socket.emit('img reply', result[0].img_url);
      } catch (e) {
        console.log('[getProductFromMomoCompare] result is not JSON');
      } 
    });
  }
    ...
    http.get(send_url, function(response){
      ...
      response.on('end', function(){
          ...
          try{
            ...
            socket.emit('search reply', responseString+ "還有什麼可以替您服務的嗎？");
...
//Weather Icon table:   https://www.developer.aero/Weather-API/Weather-API-Documentation/Weather-Developers-Documentation/Weather-Icons
//Weather API table:    https://opendata.cwb.gov.tw/opendatadoc/MFC/D0047.pdf
...
function phoneRequest(userOption){
  ...
  py.stdout.on('data', function(data){
    dataString = data.toString();
    console.log("[phoneRequest] dataString:"+dataString);
  });
  py.stdout.on('end', function(){      
...
function sendSlackMessage(text) { 
  requestLib.post({
    ...
    // url:     "https://hooks.slack.com/services/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",//Felix
    url:     "https://hooks.slack.com/services/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",
...
function getInfo(event_query,number_query,whole_query){    
  if(event_query==="阿宏之聲"){
    ...
    if(number_query!=-1 && number_query-1 < len){
      ...
      return "抱歉，資料庫目前只收錄到阿宏之聲第"+len+"期!想知道更多，問阿宏就對啦!";
    }
